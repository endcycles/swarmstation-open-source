...
prepare_app_directory() {
    ...
    case $OPTIMIZATION in
        standard)
            ...
            echo "Using standard build (no optimization)"
            ;;
...
build_mac() {
    ...
    if [ "$OPTIMIZATION" = "minimal" ]; then
        ...
        # Copy files to root for standard build
        cp dist/index.html .
        cp dist/mvp.*.js .
        cp dist/mvp.*.css . 2>/dev/null || true
...
post_build() {
    ...
    # Clean up root directory if we copied files there
    if [ "$OPTIMIZATION" = "standard" ]; then
        rm -f index.html mvp.*.js mvp.*.css
    fi
